...
public class NavBar extends HBox {
    ...
    public NavBar() {
        ...
        archiveBtn.setOnAction(e -> { if(onArchiveClick != null) onArchiveClick.run(); });
        ...
        this.getChildren().addAll(
            homeBtn, archiveBtn, searchBtn,
            spacer, 
            settingsBtn, friendsBtn, notifBtn, logoLabel
        );
    }

    public void setOnHomeClick(Runnable action) { this.onHomeClick = action; }
    public void setOnArchiveClick(Runnable action) { this.onArchiveClick = action; }
    ...
    private Button createIconButton(String svgPathData) {
        ...
        btn.setGraphic(path); 
        btn.setStyle("-fx-background-color: " + Theme.SECONDARY_COLOR + "; -fx-background-radius: 50%; -fx-min-width: 40px; -fx-min-height: 40px; -fx-padding: 0;");
